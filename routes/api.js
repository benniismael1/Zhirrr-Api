...
try {
var zahirr = db.get("beniismael");
} catch (e) {
	console.log('')
}

var creatorList = ['@sekhaa','@BYY','@BYY','@BYY', '@BYYsayang','@BYY','@Benniismael'];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------beniismael'+'Ben--GANS';
...
router.get('/yutub/playmp4', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'beniismael') return res.json(loghandler.invalidKey)
    if (!q) return res.json({ status : false, creator : `benniismael`, message : "masukan parameter q"})
...
router.get('/textmaker/make', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'beniismael') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'typography' && theme != 'underweb') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/textmaker/editing', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'beniismael') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'graffiti' && theme != 'carved') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/randomanime', async (req, res, next) => {
        var apikeyInput = req.query.apikey
            randomanime = req.query.randomanime
...
router.get('/tanggaljadian', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'beniismael') return res.json(loghandler.invalidKey)
        if (!tgl) return res.json(loghandler.notnama)
        if (!bln) return res.json(loghandler.notnama)
    if (!thn) return res.json({ status : false, creator : `${creator}`, message : "masukan tgl, bulan, thn jadian"})
